INDEX AWAL YANG MASIH BENAR (SEBELUM DIEDIT)
...
<body style="background-color: #fff8f0" x-data="{ openLogin: false, openRegister: false, openForgotPassword: false }">
    ...
    document.getElementById('loginForm').addEventListener('submit', function(event) {
        ...
        fetch(this.action, {
            ...
            .then((response) => {
                console.log(response); // Cek status respons
                if (!response.ok) {
                    throw response.json(); // Lempar error jika status tidak OK
                }
                return response.json();
            })
            .then((data) => {
                console.log(data); // Debug respons JSON
                if (!data.redirect) {
                ...
                Swal.fire({
                    ...
                    window.location.href = data.redirect; // Arahkan ke dashboard
                });
            })
            .catch(async (error) => {
                const errorData = await error; // Tunggu respons JSON error
                console.error(errorData); // Debug data error
                Swal.fire({
                    title: 'Oops!',
                    ...
                    if (errorData.redirect) {
                        window.location.href = errorData.redirect; // Arahkan ke halaman register
                    }
                });
            });
    });
            ...
            if (data.errors) {
                ...
                if (data.errors.email) {
                    Swal.fire({
                        ...
                        text: 'The email address you entered is already associated with another account. Please use a different email.',
                        icon: 'error',
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Akses properti dari Alpine.js
                        const modalContainer = document.querySelector('[x-data]');
                        if (modalContainer) {
                            modalContainer.__x.$data.openRegister = false; // Menutup modal register
                            modalContainer.__x.$data.openLogin = true;    // Membuka modal login
                        }
                    }
                });
            }
    ...
    $('#loginButton').click(function(event) {
        ...
        $.ajax({
            url: '/login',  // URL yang sesuai dengan route login
            method: 'POST',
            data: {
                email: email,
                password: password,
                _token: $('meta[name="csrf-token"]').attr('content'), // Menambahkan token CSRF
            },
            ...
            error: function(xhr, status, error) {
                // Tangani error (misalnya, kesalahan jaringan)
